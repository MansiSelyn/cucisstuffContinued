...
try {
    ...
    if (!$adminCheck->fetchColumn()) { header("Location: main.php"); exit(); }
    $isAdmin = true;
    ...
    if (!empty($_GET['get_item_data'])) {
        ...
        if (!$item) { echo json_encode(['error'=>'Termék nem található']); exit; }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['delete_user'], $_POST['user_id'])) {
            ...
            } else { $error = "Saját magad nem törölheted!"; }
        }
    ...
    foreach (array_keys($counts) as $tbl) {
        try { $counts[$tbl] = $conn->query("SELECT COUNT(*) FROM $tbl")->fetchColumn(); }
    ...
    if ($view==='items' && !$editId) {
        ...
        $s->bindValue(':o',$offset,PDO::PARAM_INT); $s->bindValue(':l',$perPage,PDO::PARAM_INT);
        $s->execute(); $items = $s->fetchAll(PDO::FETCH_ASSOC);
        ...
        $s->bindValue(':o',$offset,PDO::PARAM_INT); $s->bindValue(':l',$perPage,PDO::PARAM_INT);
        $s->execute(); $users = $s->fetchAll(PDO::FETCH_ASSOC);
    } elseif ($view==='reports') {
        ...
        $s->bindValue(':o',$offset,PDO::PARAM_INT); $s->bindValue(':l',$perPage,PDO::PARAM_INT);
        $s->execute(); $reports = $s->fetchAll(PDO::FETCH_ASSOC);
    }

    $editItem = $editUser = null;
    if ($editId) {
        if ($view==='items') {
            $s=$conn->prepare("SELECT * FROM items WHERE id=?"); $s->execute([$editId]); $editItem=$s->fetch(PDO::FETCH_ASSOC);
        } elseif ($view==='users') {
            $s=$conn->prepare("SELECT id,username,email FROM users WHERE id=?"); $s->execute([$editId]); $editUser=$s->fetch(PDO::FETCH_ASSOC);
        }
    }
} catch(PDOException $e) {
    $error = "DB HIBA: ".$e->getMessage();
    $totalPages = 0; $view = 'main';
    $items=$users=$reports=[]; $editItem=$editUser=null; $counts=['users'=>0,'items'=>0,'reports'=>0];
}

// Segédfüggvény: lapozó link
function pgLink($v,$p) { return "admin.php?view=$v&page=$p"; }
?><!DOCTYPE html>
<html lang="hu">
<head>
<meta charset="UTF-8">
...
.glow { text-shadow: var(--c-glow); }
.glow-strong { text-shadow: var(--c-glow-strong); }
...
.chrome-top-left { color: var(--c-green); letter-spacing: 2px; }
.chrome-top-left span { color: var(--c-muted); }

.chrome-top-right {
    display: flex; gap: 16px; align-items: center;
    color: var(--c-muted); font-size: 0.95rem;
}
.chrome-top-right .live-clock { color: var(--c-green); letter-spacing: 1px; }
...
.nav-btn:first-child { border-left: 1px solid var(--c-border); }
.nav-btn:hover { color: var(--c-green); background: rgba(57,255,20,0.04); }
.nav-btn.active {
    color: var(--c-green);
...
.nav-btn.active .nav-badge { background: var(--c-green); }
.nav-btn.nav-back { margin-left: auto; color: var(--c-muted); }
.nav-btn.nav-back:hover { color: var(--c-amber); }
...
.theme-btn:hover { background: var(--c-border); border-color: var(--c-green); box-shadow: var(--c-glow); }
...
.banner-ok  { border-color: var(--c-green); background: rgba(57,255,20,0.05); color: var(--c-green); }
.banner-err { border-color: var(--c-red);   background: rgba(255,34,0,0.07);  color: var(--c-red); }
.banner::before { margin-right: 10px; }
.banner-ok::before  { content: '[OK]'; }
.banner-err::before { content: '[ERR]'; }
...
.data-table th::before { content: '> '; color: var(--c-green-dim); }
.data-table td {
    padding: 9px 14px;
    border-bottom: 1px solid var(--c-border);
    color: var(--c-text);
...
.data-table tr:hover td { background: rgba(57,255,20,0.03); color: var(--c-green); }
.data-table td.wrap { white-space: normal; max-width: 260px; word-break: break-word; }
.data-table td.mono { font-family: var(--font-mono); font-size: 0.75rem; color: var(--c-muted); }
...
.act-edit  { color: #5599ff; border-color: #2244aa; }
.act-edit:hover  { background: rgba(85,153,255,0.1); border-color: #5599ff; box-shadow: 0 0 8px rgba(85,153,255,0.3); }
.act-del   { color: var(--c-red); border-color: #660000; }
.act-del:hover   { background: rgba(255,34,0,0.1); border-color: var(--c-red); box-shadow: 0 0 8px rgba(255,34,0,0.3); }
.act-view  { color: var(--c-green-mid); border-color: var(--c-border2); }
.act-view:hover  { background: rgba(57,255,20,0.07); border-color: var(--c-green-mid); }
...
.empty-state::before { content: '[ NO DATA ]'; display: block; font-size: 1.8rem; color: var(--c-green-dim); margin-bottom: 8px; }
...
.edit-terminal-header::before { content: '▶'; font-size: 1rem; color: var(--c-green-dim); }
.edit-terminal-body { padding: 20px 20px 16px; }
...
.field-label::before { content: ':: '; color: var(--c-green-dim); }
...
.field-input::placeholder, .field-textarea::placeholder { color: var(--c-green-dim); }
...
.btn-save:hover { background: rgba(57,255,20,0.2); box-shadow: var(--c-glow-strong); }
.btn-cancel {
    padding: 8px 20px;
    background: transparent;
    border: 1px solid var(--c-border2);
...
.btn-cancel:hover { border-color: var(--c-amber); color: var(--c-amber); }
...
.dash-card:hover { border-color: var(--c-green-mid); }
.dash-card:hover .dash-number { text-shadow: var(--c-glow-strong); }
.dash-label {
    font-size: 0.68rem;
    letter-spacing: 3px;
    text-transform: uppercase;
...
.dash-label::before { content: '■ '; font-size: 0.5rem; }
.dash-number {
    font-family: var(--font-vt);
    font-size: 3.2rem;
    color: var(--c-green);
...
.dash-sublabel { font-size: 0.7rem; color: var(--c-green-dim); margin-top: 6px; letter-spacing: 1px; }
...
.sys-info p::before { content: '> '; color: var(--c-green-dim); }
.sys-info strong { color: var(--c-green-mid); }
...
.pg-btn:hover:not(.disabled) { border-color: var(--c-green); color: var(--c-green); box-shadow: var(--c-glow); }
.pg-btn.disabled { opacity: 0.25; pointer-events: none; }
.pg-info { font-family: var(--font-vt); font-size: 1.2rem; color: var(--c-green); min-width: 80px; text-align: center; letter-spacing: 2px; }
...
.pm-btn:hover { background: var(--c-green-mid); color: var(--c-bg); }
...
.product-menu-content.show { display: block; }
.product-menu-item {
    width: 100%; padding: 7px 12px;
    background: transparent; border: none;
    color: var(--c-text);
    font-family: var(--font-mono); font-size: 0.78rem;
    letter-spacing: 1px; text-align: left; cursor: pointer;
    transition: all 0.15s;
    text-transform: uppercase;
}
.product-menu-item:hover { background: rgba(57,255,20,0.07); color: var(--c-green); }
.product-menu-item.delete:hover { background: rgba(255,34,0,0.1); color: var(--c-red); }
...
.product-main-image-container {
    ...
    overflow: hidden; margin-bottom: 12px; min-height: 200px;
}
.product-main-image { max-width: 100%; max-height: 100%; object-fit: contain; cursor: zoom-in; }
.product-no-image-placeholder { color: var(--c-green-dim); font-family: var(--font-vt); font-size: 1.4rem; letter-spacing: 3px; }
.gallery-nav {
    position: absolute; top: 50%; transform: translateY(-50%);
    background: rgba(0,0,0,0.8); color: var(--c-green); border: 1px solid var(--c-green-mid);
    width: 36px; height: 36px; cursor: pointer;
    display: flex; align-items: center; justify-content: center; font-size: 1.2rem;
    transition: all 0.15s; z-index: 10;
}
.gallery-nav:hover { background: var(--c-green-mid); color: var(--c-bg); }
.gallery-nav.prev { left: 8px; }
.gallery-nav.next { right: 8px; }
.gallery-nav.hidden { display: none; }
.product-thumbnails { display: flex; gap: 8px; overflow-x: auto; padding: 4px 0; min-height: 72px; }
.product-thumbnail {
...
.product-title { font-family: var(--font-vt); font-size: 2rem; color: var(--c-green); letter-spacing: 2px; text-shadow: var(--c-glow); word-break: break-word; }
.product-price { font-family: var(--font-vt); font-size: 2.6rem; color: var(--c-green); text-shadow: var(--c-glow-strong); letter-spacing: 3px; }
.product-seller { font-size: 0.85rem; color: var(--c-muted); }
.product-seller strong { color: var(--c-green-mid); }
.product-date { font-size: 0.75rem; color: var(--c-green-dim); letter-spacing: 1px; }
.product-description {
    font-size: 0.85rem; line-height: 1.8; color: var(--c-text);
    background: rgba(0,0,0,0.4); padding: 14px; border: 1px solid var(--c-border);
    max-height: 260px; overflow-y: auto; white-space: pre-wrap;
}
.product-buy-btn {
    background: rgba(0,180,60,0.12); border: 1px solid #00aa3a;
    color: #00ff66; font-family: var(--font-mono); font-size: 0.85rem;
    letter-spacing: 3px; text-transform: uppercase; padding: 14px;
    cursor: pointer; transition: all 0.2s; margin-top: auto;
}
.product-buy-btn:hover { background: rgba(0,180,60,0.2); box-shadow: 0 0 20px rgba(0,180,60,0.3); }
...
.lightbox-content { display: flex; align-items: flex-start; gap: 10px; max-width: 95vw; max-height: 95vh; }
.lightbox-image { max-width: calc(95vw - 60px); max-height: 95vh; object-fit: contain; border: 1px solid var(--c-green-mid); }
.lightbox-close {
    background: rgba(0,0,0,0.9); border: 1px solid var(--c-green-mid);
    color: var(--c-green); font-size: 1.5rem; cursor: pointer;
    width: 40px; height: 40px; display: flex; align-items: center; justify-content: center;
    flex-shrink: 0; transition: all 0.15s;
}
.lightbox-close:hover { background: var(--c-green-mid); color: var(--c-bg); }

/* view-item-btn (report táblában) */
.view-item-btn {
    background: none; border: none;
    color: var(--c-green-mid); cursor: pointer;
    font-family: var(--font-mono); font-size: 0.83rem;
    text-decoration: underline; letter-spacing: 0.5px;
    transition: color 0.15s;
}
.view-item-btn:hover { color: var(--c-green); }

/* Responsive */
@media (max-width: 1100px) {
    .product-modal-card { grid-template-columns: 1fr; overflow-y: auto; }
    .product-gallery { height: 45vh; border-right: none; border-bottom: 1px solid var(--c-border2); }
}
@media (max-width: 600px) {
    .nav-btn .nav-label { display: none; }
    .product-title { font-size: 1.5rem; }
    .product-price { font-size: 2rem; }
    .chrome-top-right .sys-time-full { display: none; }
}
</style>
</head>
<body>
...
<div class="terminal-body">
    ...
    <?php elseif($view==='reports'): ?>
        ...
        <div class="data-panel">
            <table class="data-table">
                <thead><tr>
                    <th>ID</th><th>TERMÉK</th><th>BEJELENTŐ</th><th>TULAJDONOS</th><th>INDOK</th><th>DÁTUM</th><th>OPS</th>
                </tr></thead>
                <tbody>
    ...
    <?php elseif($view==='users'): ?>
        ...
        <div class="data-panel">
            <table class="data-table">
                <thead><tr>
                    <th>ID</th><th>USERNAME</th><th>EMAIL</th><th>ADMIN</th><th>ITEMS</th><th>REGDÁTUM</th><th>OPS</th>
                </tr></thead>
                <tbody>
    ...
    <?php elseif($view==='items'): ?>
        ...
        <div class="data-panel">
            <table class="data-table">
                <thead><tr>
                    <th>ID</th><th>CÍM</th><th>ELADÓ</th><th>ÁR</th><th>LEÍRÁS</th><th>DÁTUM</th><th>OPS</th>
                </tr></thead>
                <tbody>
...
// ── TÉMA ──
(function(){
    const KEY='admin_theme', body=document.body, btn=document.getElementById('themeToggleBtn');
    function apply(t){ body.classList.toggle('light-mode',t==='light'); localStorage.setItem(KEY,t); btn.textContent=t==='light'?'DARK':'LIGHT'; }
...
setTimeout(()=>document.querySelectorAll('.banner').forEach(el=>{el.style.opacity='0';el.style.transition='opacity .5s';setTimeout(()=>el.remove(),500);}),5000);

// ── TÖRLÉSEK ──
const CONFIRM_MSG = { user:'Biztosan törlöd a felhasználót? Minden adata elvész!', item:'Biztosan törlöd a terméket?', report:'Biztosan törlöd a reportot?' };
const POST_KEY    = { user:'delete_user=1&user_id=', item:'delete_item=1&item_id=', report:'delete_report=1&report_id=' };
function doDelete(type,id){
    if(!confirm(CONFIRM_MSG[type])) return;
    fetch('admin.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:POST_KEY[type]+id}).then(()=>location.reload());
}
...
let imgs=[], imgIdx=0, prodId=null, prodUid=null;

function setImg(i){
    if(i>=0&&i<imgs.length){ pm.mainImg.style.display='block'; pm.noImg.style.display='none'; pm.mainImg.src=imgs[i]; imgIdx=i; pm.mainImg.onload=adjustH; pm.mainImg.onerror=()=>{pm.mainImg.style.display='none';pm.noImg.style.display='block';}; document.querySelectorAll('.product-thumbnail').forEach((t,j)=>t.classList.toggle('active',j===i)); }
    else { pm.mainImg.style.display='none'; pm.noImg.style.display='block'; }
}
function adjustH(){
    const c=document.querySelector('.product-main-image-container'), g=document.querySelector('.product-gallery'), tb=document.querySelector('.product-thumbnails');
    if(c&&g){ const avail=g.clientHeight-32-(tb?tb.offsetHeight:72)-16; c.style.height=Math.max(200,Math.min(pm.mainImg.naturalHeight||avail,avail))+'px'; }
}
function openPM(){ pm.modal.classList.add('active'); document.body.style.overflow='hidden'; setTimeout(adjustH,100); }
function closePM(){ if(pm.lbOver.classList.contains('active')) pm.lbOver.classList.remove('active'); pm.modal.classList.remove('active'); document.body.style.overflow=''; }
...
document.querySelectorAll('.view-item-btn').forEach(btn=>btn.addEventListener('click',function(e){
    ...
    fetch('admin.php?get_item_data='+this.dataset.itemId).then(r=>r.json()).then(d=>{
        ...
        prodId=d.id; prodUid=d.user_id; imgs=d.images; imgIdx=0;
        pm.title.textContent=d.title; pm.price.textContent=d.price;
        pm.seller.innerHTML='Eladó: <strong>'+d.seller+'</strong>';
        pm.date.textContent=d.date; pm.desc.textContent=d.description;
        pm.thumbs.innerHTML='';
        if(imgs.length){ imgs.forEach((src,i)=>{ const t=document.createElement('div'); t.className='product-thumbnail'+(i===0?' active':''); t.innerHTML='<img src="'+src+'" alt="">'; t.onclick=e=>{e.stopPropagation();setImg(i);}; pm.thumbs.appendChild(t); }); setImg(0); } else setImg(-1);
        pm.prev.classList.toggle('hidden',imgs.length<2); pm.next.classList.toggle('hidden',imgs.length<2);
        <?php if(isset($_SESSION['user_id'])): ?>
        const isOwner=parseInt(d.user_id)===<?= (int)$_SESSION['user_id'] ?>;
        if(<?= $isAdmin?'true':'false' ?>){ pm.menuCont.style.display='block'; pm.reportBtn.style.display='none'; pm.editBtn.style.display='block'; pm.delBtn.style.display='block'; pm.editBtn.onclick=()=>location.href='admin.php?view=items&id='+prodId; pm.delBtn.onclick=()=>{ if(confirm('Biztosan törlöd ezt a terméket?')){ const f=document.createElement('form'); f.method='POST'; f.innerHTML='<input type="hidden" name="item_id" value="'+d.id+'"><input type="hidden" name="delete_item" value="1">'; document.body.appendChild(f); f.submit(); }}; } else if(!isOwner){ pm.menuCont.style.display='block'; pm.reportBtn.style.display='block'; pm.editBtn.style.display='none'; pm.delBtn.style.display='none'; } else pm.menuCont.style.display='none';
        <?php endif; ?>
        openPM();
    }).catch(()=>alert('Betöltési hiba!'));
}));

function toggleProductMenu(btn){ const m=btn.nextElementSibling; m.classList.toggle('show'); document.querySelectorAll('.product-menu-content').forEach(x=>{ if(x!==m) x.classList.remove('show'); }); }
pm.closeBtn.addEventListener('click',closePM);
pm.modal.addEventListener('click',e=>{ if(e.target===pm.modal) closePM(); });
pm.prev.addEventListener('click',e=>{ e.stopPropagation(); setImg(imgIdx-1>=0?imgIdx-1:imgs.length-1); });
pm.next.addEventListener('click',e=>{ e.stopPropagation(); setImg(imgIdx+1<imgs.length?imgIdx+1:0); });
pm.mainImg.addEventListener('click',()=>{ if(pm.mainImg.src&&pm.mainImg.style.display!=='none'){ pm.lbImg.src=pm.mainImg.src; pm.lbOver.classList.add('active'); } });
pm.lbClose.addEventListener('click',()=>pm.lbOver.classList.remove('active'));
pm.lbOver.addEventListener('click',e=>{ if(e.target===pm.lbOver) pm.lbOver.classList.remove('active'); });
document.addEventListener('keydown',e=>{ if(e.key==='Escape'){ if(pm.lbOver.classList.contains('active')) pm.lbOver.classList.remove('active'); else if(pm.modal.classList.contains('active')) closePM(); } });
window.addEventListener('resize',()=>{ if(pm.modal.classList.contains('active')) adjustH(); });
